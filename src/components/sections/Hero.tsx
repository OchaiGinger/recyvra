...
export const Hero = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ctx.revert();
      introHeadlineChars.current = []; // Clear refs on unmount/re-run
    };
  }, []);
  ...
  return (
    <section ref={containerRef} className="relative w-full h-[620vh]">
      ...
      >
        ...
        <div
          ref={logoRef}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[clamp(240px,50vw,600px)] aspect-square z-2 pointer-events-none opacity-0"
        >
          <RecyvraR />
        </div>
        ...
        <div ref={headlineRef} className="relative z-40 text-center px-[5%]">
          <h1 className="font-headline font-light leading-[1.1] m-0">
            <span className="block text-[clamp(2rem,7vw,6rem)]">
          ...
          <div ref={subtitleRef} className="mt-6 md:mt-8 opacity-0">
            <p className="font-body text-[clamp(0.75rem,1.2vw,1.1rem)] text-[#e8e0d0]/50 tracking-wider px-4">
        ...
        <button
          ref={ctaBtnRef}
          className="absolute bottom-[10%] md:bottom-[15%] left-1/2 -translate-x-1/2 z-50 px-8 md:px-12 py-3 md:py-4 text-[#e8e0d0] font-bold tracking-[0.2em] text-[0.75rem] md:text-[0.85rem] uppercase rounded-full bg-white/5 backdrop-blur-[32px] border border-white/10 hover:bg-[#c9a96e]/20 hover:border-[#c9a96e]/40 transition-all duration-500 shadow-2xl opacity-0 pointer-events-auto cursor-pointer"
          onClick={() =>
            document
        ...
        <div
          ref={aboutRef}
          className="absolute inset-0 flex items-center justify-center opacity-0 z-15 pointer-events-none px-[10%]"
        >
          <p className="font-headline text-[clamp(1.2rem,3.5vw,3rem)] text-[#e8e0d0] text-center leading-relaxed max-w-250 mx-auto">
            {(() => {
        ...
        <div className="hidden lg:block">
          {ORBIT_CARDS.map((card, i) => (
            <div
              ...
              className="absolute flex items-center gap-6 bg-[#111e18]/60 backdrop-blur-[32px] border border-white/10 rounded-3xl p-8 opacity-0 w-[clamp(300px,34vw,430px)] transition-all duration-500 hover:bg-[#111e18]/80 hover:border-[#4ecca3]/30 hover:-translate-y-2 z-25"
              style={{ top: card.pos.top, left: card.pos.left }}
            >
              <div
                className="shrink-0w-16 h-16 rounded-md flex items-center justify-center text-[#f5f0e8] bg-white/5 border border-white/10"
                style={{
              ...
              <p className="text-sm md:text-base text-white/80 leading-relaxed m-0">
                <strong className="text-white block mb-1 font-headline tracking-wide">
                  {card.bold}
                </strong>
                {card.rest}
              </p>
